...
class DashboardService():
    ...
    def get_all_data_criacao(self):
        ...
        total = []
        try:
            query = """SELECT COUNT(*) as total,DATE_FORMAT(created_at,"%d-%m-%Y") AS data FROM `funcionario` GROUP BY DATE(created_at)"""
            cursor = connection.cursor()
            cursor.execute(query)
            totais = cursor.fetchall()
            cursor.close()
    ...
    def get_all_presenca_criacao(self):
        data = []
        total = []
        try:
            query = """SELECT COUNT(*) as total,DATE_FORMAT(created_at,"%d-%m-%Y") AS data FROM `presenca` Where presente = 1 GROUP BY DATE(created_at)"""
            cursor = connection.cursor()
            cursor.execute(query)
            totais = cursor.fetchall()
            cursor.close()
